...
1. *OPTIONAL* Rerun codegen targeting Microkit
    ...
    Launch the Slash script [micro-examples/microkit/port_queues/event_data_base_types/aadl/bin/run-hamr.cmd](aadl/bin/run-hamr.cmd) from the command line, passing `Microkit` as the platform argument.

   ```
   micro-examples/microkit/port_queues/event_data_base_types/aadl/bin/run-hamr.cmd Microkit
   ```

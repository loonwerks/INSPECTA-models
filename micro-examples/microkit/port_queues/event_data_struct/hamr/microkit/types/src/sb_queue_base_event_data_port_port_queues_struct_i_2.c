...
#include <sb_queue_base_event_data_port_port_queues_struct_i_2.h>

//------------------------------------------------------------------------------
// Sender API
//
// See sb_queue_base_event_data_port_port_queues_struct_i_2.h for API documentation. Only implementation details are documented here.

void sb_queue_base_event_data_port_port_queues_struct_i_2_init(sb_queue_base_event_data_port_port_queues_struct_i_2_t *queue) {
...
void sb_queue_base_event_data_port_port_queues_struct_i_2_enqueue(
  sb_queue_base_event_data_port_port_queues_struct_i_2_t *queue,
  ...
  // elt[queue->numSent % SB_QUEUE_BASE_EVENT_DATA_PORT_PORT_QUEUES_STRUCT_I_2_SIZE]
  // is always considered dirty. So do not advance queue->NumSent
  // till AFTER data is copied.

  size_t index = queue->numSent % SB_QUEUE_BASE_EVENT_DATA_PORT_PORT_QUEUES_STRUCT_I_2_SIZE;
...
void sb_queue_base_event_data_port_port_queues_struct_i_2_Recv_init(
  sb_queue_base_event_data_port_port_queues_struct_i_2_Recv_t *recvQueue,
  sb_queue_base_event_data_port_port_queues_struct_i_2_t *queue) {
...
bool sb_queue_base_event_data_port_port_queues_struct_i_2_dequeue(
  sb_queue_base_event_data_port_port_queues_struct_i_2_Recv_t *recvQueue,
  sb_event_counter_t *numDropped,
  base_event_data_port_port_queues_struct_i *data) {

  sb_event_counter_t *numRecv = &recvQueue->numRecv;
  sb_queue_base_event_data_port_port_queues_struct_i_2_t *queue = recvQueue->queue;
  ...
  // incremented. Thus there are really only (SB_QUEUE_BASE_EVENT_DATA_PORT_PORT_QUEUES_STRUCT_I_2_SIZE - 1)
  // elements in the queue.
  *numDropped = (numNew <= SB_QUEUE_BASE_EVENT_DATA_PORT_PORT_QUEUES_STRUCT_I_2_SIZE - 1) ? 0 : numNew - SB_QUEUE_BASE_EVENT_DATA_PORT_PORT_QUEUES_STRUCT_I_2_SIZE + 1;
  ...
  *numRecv += *numDropped + 1;
  ...
  size_t index = (*numRecv - 1) % SB_QUEUE_BASE_EVENT_DATA_PORT_PORT_QUEUES_STRUCT_I_2_SIZE;
  *data = queue->elt[index]; // Copy data

  // Acquire memory fence - ensure read of data BEFORE reading queue->numSent again
  __atomic_thread_fence(__ATOMIC_ACQUIRE);

  if (queue->numSent - *numRecv + 1 < SB_QUEUE_BASE_EVENT_DATA_PORT_PORT_QUEUES_STRUCT_I_2_SIZE) {
...
bool sb_queue_base_event_data_port_port_queues_struct_i_2_is_empty(sb_queue_base_event_data_port_port_queues_struct_i_2_Recv_t *recvQueue) {

#include "consumer_p_s1_consumer.h"

// Do not edit this file as it will be overwritten if HAMR codegen is rerun

void consumer_p_s1_consumer_initialize(void);
void handle_read_port(void);
void consumer_p_s1_consumer_notify(microkit_channel channel);

volatile sb_queue_event_data_struct_port_queues_struct_i_1_t *read_port_queue_1;
sb_queue_event_data_struct_port_queues_struct_i_1_Recv_t read_port_recv_queue;

#define PORT_FROM_MON 58

bool read_port_is_empty(void) {
  return sb_queue_event_data_struct_port_queues_struct_i_1_is_empty(&read_port_recv_queue);
}

bool get_read_port_poll(sb_event_counter_t *numDropped, event_data_struct_port_queues_struct_i *data) {
  return sb_queue_event_data_struct_port_queues_struct_i_1_dequeue((sb_queue_event_data_struct_port_queues_struct_i_1_Recv_t *) &read_port_recv_queue, numDropped, data);
}

bool get_read_port(event_data_struct_port_queues_struct_i *data) {
  sb_event_counter_t numDropped;
  return get_read_port_poll (&numDropped, data);
}

uintmax_t get_read_port_num_invalid(void) {
  return sb_queue_event_data_struct_port_queues_struct_i_1_numInvalid((sb_queue_event_data_struct_port_queues_struct_i_1_Recv_t *) &read_port_recv_queue);
}

void init(void) {
  sb_queue_event_data_struct_port_queues_struct_i_1_Recv_init(&read_port_recv_queue, (sb_queue_event_data_struct_port_queues_struct_i_1_t *) read_port_queue_1);

  consumer_p_s1_consumer_initialize();
}

void notified(microkit_channel channel) {
  switch (channel) {
    case PORT_FROM_MON:
      if (!read_port_is_empty()) {
        handle_read_port();
      }
      break;
    default:
      consumer_p_s1_consumer_notify(channel);
  }
}

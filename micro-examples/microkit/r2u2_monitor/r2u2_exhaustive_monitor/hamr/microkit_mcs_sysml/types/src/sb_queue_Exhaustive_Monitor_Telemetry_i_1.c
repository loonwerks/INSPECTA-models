...
// Sender API
//
// See sb_queue_Exhaustive_Monitor_Telemetry_i_1.h for API documentation. Only implementation details are documented here.
...
void sb_queue_Exhaustive_Monitor_Telemetry_i_1_enqueue(
  ...
  // elt[queue->numSent % SB_QUEUE_EXHAUSTIVE_MONITOR_TELEMETRY_I_1_SIZE]
  // is always considered dirty. So do not advance queue->NumSent
  // till AFTER data is copied.

  size_t index = queue->numSent % SB_QUEUE_EXHAUSTIVE_MONITOR_TELEMETRY_I_1_SIZE;
...
bool sb_queue_Exhaustive_Monitor_Telemetry_i_1_peek_latest(
  ...
  sb_queue_Exhaustive_Monitor_Telemetry_i_1_Recv_t snapshot = {
    .numRecv = numSent - 1,
    .queue = queue
  };
  sb_event_counter_t numDropped;
  return sb_queue_Exhaustive_Monitor_Telemetry_i_1_dequeue(&snapshot, &numDropped, data);
}
...
bool sb_queue_Exhaustive_Monitor_Telemetry_i_1_dequeue(
  ...
  sb_queue_Exhaustive_Monitor_Telemetry_i_1_t *queue = recvQueue->queue;
  ...
  // incremented. Thus there are really only (SB_QUEUE_EXHAUSTIVE_MONITOR_TELEMETRY_I_1_SIZE - 1)
  // elements in the queue.
  *numDropped = (numNew <= SB_QUEUE_EXHAUSTIVE_MONITOR_TELEMETRY_I_1_SIZE - 1) ? 0 : numNew - SB_QUEUE_EXHAUSTIVE_MONITOR_TELEMETRY_I_1_SIZE + 1;
  ...
  *numRecv += *numDropped + 1;
  ...
  size_t index = (*numRecv - 1) % SB_QUEUE_EXHAUSTIVE_MONITOR_TELEMETRY_I_1_SIZE;
  ...
  if (queue->numSent - *numRecv + 1 < SB_QUEUE_EXHAUSTIVE_MONITOR_TELEMETRY_I_1_SIZE) {
    ...
    if (!Exhaustive_Monitor_Telemetry_i_is_valid(&staging)) {
...
bool sb_queue_Exhaustive_Monitor_Telemetry_i_1_peek(
  ...
  sb_queue_Exhaustive_Monitor_Telemetry_i_1_Recv_t snapshot = *recvQueue;
  return sb_queue_Exhaustive_Monitor_Telemetry_i_1_dequeue(&snapshot, numDropped, data);
}

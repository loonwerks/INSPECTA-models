...
#include <sb_queue_Exhaustive_Monitor_C_Samples_1.h>
...
// Sender API
//
// See sb_queue_Exhaustive_Monitor_C_Samples_1.h for API documentation. Only implementation details are documented here.
...
void sb_queue_Exhaustive_Monitor_C_Samples_1_enqueue(
  ...
  // elt[queue->numSent % SB_QUEUE_EXHAUSTIVE_MONITOR_C_SAMPLES_1_SIZE]
  // is always considered dirty. So do not advance queue->NumSent
  // till AFTER data is copied.

  size_t index = queue->numSent % SB_QUEUE_EXHAUSTIVE_MONITOR_C_SAMPLES_1_SIZE;

  memcpy(&queue->elt[index], data, Exhaustive_Monitor_C_Samples_BYTE_SIZE); // Copy data into queue
...
bool sb_queue_Exhaustive_Monitor_C_Samples_1_peek_latest(
  ...
  sb_queue_Exhaustive_Monitor_C_Samples_1_Recv_t snapshot = {
    .numRecv = numSent - 1,
    .queue = queue
  };
  sb_event_counter_t numDropped;
  return sb_queue_Exhaustive_Monitor_C_Samples_1_dequeue(&snapshot, &numDropped, data);
}
...
bool sb_queue_Exhaustive_Monitor_C_Samples_1_dequeue(
  ...
  sb_queue_Exhaustive_Monitor_C_Samples_1_t *queue = recvQueue->queue;
  ...
  // incremented. Thus there are really only (SB_QUEUE_EXHAUSTIVE_MONITOR_C_SAMPLES_1_SIZE - 1)
  // elements in the queue.
  *numDropped = (numNew <= SB_QUEUE_EXHAUSTIVE_MONITOR_C_SAMPLES_1_SIZE - 1) ? 0 : numNew - SB_QUEUE_EXHAUSTIVE_MONITOR_C_SAMPLES_1_SIZE + 1;
  ...
  *numRecv += *numDropped + 1;
  ...
  size_t index = (*numRecv - 1) % SB_QUEUE_EXHAUSTIVE_MONITOR_C_SAMPLES_1_SIZE;
  memcpy(data, &queue->elt[index], Exhaustive_Monitor_C_Samples_BYTE_SIZE); // Copy data

  // Acquire memory fence - ensure read of data BEFORE reading queue->numSent again
  __atomic_thread_fence(__ATOMIC_ACQUIRE);

  if (queue->numSent - *numRecv + 1 < SB_QUEUE_EXHAUSTIVE_MONITOR_C_SAMPLES_1_SIZE) {
...
bool sb_queue_Exhaustive_Monitor_C_Samples_1_peek(
  ...
  sb_queue_Exhaustive_Monitor_C_Samples_1_Recv_t snapshot = *recvQueue;
  return sb_queue_Exhaustive_Monitor_C_Samples_1_dequeue(&snapshot, numDropped, data);
}

...
// Removes HAMR-generated artifacts while preserving hand-written behavior code, so that
// regenerating does not clobber the demo logic these R2U2 examples exist to show.
//
// This model produces two variants from the two AADL packages: hamr/microkit_mcs from
// <Pkg>.aadl (Rust components) and hamr/microkit_mcs_c from <Pkg>_C.aadl (C components).
// hamr/microkit_mcs_sysml is produced from the SysML sibling and is left alone unless
// explicitly named. Pass the directory (or directories) to clean as arguments; with no
// arguments every variant under hamr/ is cleaned.
...
val toKeep = ISZ(
  ...
  KeepPattern("_user.c"), // microkit C user implementation files, including *_MON_user.c

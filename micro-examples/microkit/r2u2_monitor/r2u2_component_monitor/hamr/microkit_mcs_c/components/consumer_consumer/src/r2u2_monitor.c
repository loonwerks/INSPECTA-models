...
void r2u2_monitor_post_timeTriggered(void) {
  ...
  r2u2_verdict_status_t r2u2_sample_arrives_verdict = R2U2_VERDICT_UNKNOWN;
  if (r2u2_monitor.verdict_valid[0]) {
    ...
    r2u2_sample_arrives_verdict = truth ? R2U2_VERDICT_TRUE : R2U2_VERDICT_FALSE;
  }
  handle_r2u2_verdict(sample_arrives, r2u2_sample_arrives_verdict);

  // Send one result through each mapped alert port.
  if (r2u2_sample_arrives_verdict != R2U2_VERDICT_UNKNOWN) {
    bool truth = r2u2_sample_arrives_verdict == R2U2_VERDICT_TRUE;
    if (!truth) {

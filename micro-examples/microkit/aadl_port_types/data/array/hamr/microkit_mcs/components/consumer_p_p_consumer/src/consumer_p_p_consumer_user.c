...
struct data_1_prod_2_cons_array_struct_i nullElem = {42, 42};

void consumer_p_p_consumer_initialize(void) {
  printf("%s: I'm periodic\n", microkit_name);
}

bool isNullElem(data_1_prod_2_cons_array_struct_i value) {
...
void consumer_p_p_consumer_timeTriggered(void) {
  data_1_prod_2_cons_array_ArrayOfStruct value;
  bool isFresh = get_read_port(&value);
  printf("%s: retrieved [", microkit_name);
  for (int i = 0; i < data_1_prod_2_cons_array_ArrayOfStruct_DIM_0; i++) {
    if(!isNullElem(value[i])) {
      printf("(%d, %d)", value[i].currentEvent, value[i].totalEventsSent);

      if (i + 1 != data_1_prod_2_cons_array_ArrayOfStruct_DIM_0 && (!isNullElem(value[i+1]))) {
        printf(", ");
      }
    }
  }

...
bool isNullElem(event_data_2_prod_2_cons_array_struct_i value) {
  return value.currentEvent == nullElem.currentEvent && value.totalEventsSent == nullElem.totalEventsSent;
}

void printArray(event_data_2_prod_2_cons_array_ArrayOfStruct value) {
  printf("[");
  for (int i = 0; i < event_data_2_prod_2_cons_array_ArrayOfStruct_DIM_0; i++)  {
    if (!isNullElem(value[i])) {
      ...
      if (i + 1 != event_data_2_prod_2_cons_array_ArrayOfStruct_DIM_0 && (!isNullElem(value[i+1]))) {
        printf(", ");
      }
    }
  }

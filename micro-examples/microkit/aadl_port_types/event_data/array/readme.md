# AADL Event Data Ports — Array Payload

This micro-example demonstrates how connections involving AADL **event data
ports** with an **array payload** are handled for both periodic and sporadic
...
The SysML model in
[`sysml/event_data_2_prod_2_cons_array.sysml`](sysml/event_data_2_prod_2_cons_array.sysml) was
...
```c
typedef struct event_data_2_prod_2_cons_array_struct_i {
  int32_t currentEvent;
  int32_t totalEventsSent;
} event_data_2_prod_2_cons_array_struct_i;

#define event_data_2_prod_2_cons_array_ArrayOfStruct_BYTE_SIZE 80
#define event_data_2_prod_2_cons_array_ArrayOfStruct_DIM_0 10

typedef event_data_2_prod_2_cons_array_struct_i event_data_2_prod_2_cons_array_ArrayOfStruct [event_data_2_prod_2_cons_array_ArrayOfStruct_DIM_0];
```

The payload is a fixed-size C array of 10 `struct_i` elements (80 bytes
total).  The queue enqueues and dequeues a full 80-byte copy on every
send/receive.
...
```c
bool put_write_port(const event_data_2_prod_2_cons_array_ArrayOfStruct *data) {
  sb_queue_event_data_2_prod_2_cons_array_ArrayOfStruct_1_enqueue((sb_queue_event_data_2_prod_2_cons_array_ArrayOfStruct_1_t *) write_port_queue_1, (event_data_2_prod_2_cons_array_ArrayOfStruct *) data);

  return true;
}
```

`enqueue` copies the full 80-byte array into the next ring-buffer slot and
...
```c
bool get_read_port1(event_data_2_prod_2_cons_array_ArrayOfStruct *data) {
  sb_event_counter_t numDropped;
  return get_read_port1_poll (&numDropped, data);
}

bool get_read_port1_poll(sb_event_counter_t *numDropped, event_data_2_prod_2_cons_array_ArrayOfStruct *data) {
  return sb_queue_event_data_2_prod_2_cons_array_ArrayOfStruct_1_dequeue((sb_queue_event_data_2_prod_2_cons_array_ArrayOfStruct_1_Recv_t *) &read_port1_recv_queue, numDropped, data);
}
```

`dequeue` compares `numRecv` to `numSent`.  If `numSent > numRecv` it copies
the next enqueued 80-byte array into `*data`, increments `numRecv`, and
returns `true`.  If `numSent == numRecv` the queue is empty and returns
`false` without touching `*data`.

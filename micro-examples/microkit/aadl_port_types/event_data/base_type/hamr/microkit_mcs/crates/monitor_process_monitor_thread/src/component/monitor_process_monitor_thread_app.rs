...
verus! {
  ...
  impl monitor_process_monitor_thread {
        ...
        // PLACEHOLDER MARKER TIME TRIGGERED ENSURES
    {
      ...
      log_sched_state(api.get_sched_state());

      log_state(
        api.get_producer_p_p1_producer_write_port(),
        api.get_producer_p_p2_producer_write_port());
    }
  ...
  pub fn log_sched_state(s: hamr::SchedState)
  {
    log::info!("Last {:?}, Next {:?}", s.last_yielded_ch, s.next_dispatch_ch);
  }

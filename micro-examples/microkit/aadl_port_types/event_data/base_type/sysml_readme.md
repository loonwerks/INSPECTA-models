...
1. *OPTIONAL* Rerun codegen targeting Microkit
    ...
    [micro-examples/microkit/aadl_port_types/event_data/base_type/sysml/bin/run-hamr.cmd](sysml/bin/run-hamr.cmd)
    from the command line, passing `Microkit` as the platform argument.

   ```
   micro-examples/microkit/aadl_port_types/event_data/base_type/sysml/bin/run-hamr.cmd Microkit
   ```
...
1. Build and simulate the seL4 Microkit image with MCS scheduling
    ...
    docker run -it --rm -v $(pwd):/home/microkit/INSPECTA-models jasonbelt/microkit_provers bash -ci \
        "cd INSPECTA-models/micro-examples/microkit/aadl_port_types/event_data/base_type/hamr/microkit_mcs && \

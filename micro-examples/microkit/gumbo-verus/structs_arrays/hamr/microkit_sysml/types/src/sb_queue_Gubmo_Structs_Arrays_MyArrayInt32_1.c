...
// Sender API
//
// See sb_queue_Gubmo_Structs_Arrays_MyArrayInt32_1.h for API documentation. Only implementation details are documented here.

void sb_queue_Gubmo_Structs_Arrays_MyArrayInt32_1_init(sb_queue_Gubmo_Structs_Arrays_MyArrayInt32_1_t *queue) {
...
void sb_queue_Gubmo_Structs_Arrays_MyArrayInt32_1_enqueue(
  ...
  // elt[queue->numSent % SB_QUEUE_GUBMO_STRUCTS_ARRAYS_MYARRAYINT32_1_SIZE]
  // is always considered dirty. So do not advance queue->NumSent
  // till AFTER data is copied.

  size_t index = queue->numSent % SB_QUEUE_GUBMO_STRUCTS_ARRAYS_MYARRAYINT32_1_SIZE;

  memcpy(&queue->elt[index], data, Gubmo_Structs_Arrays_MyArrayInt32_BYTE_SIZE); // Copy data into queue
...
void sb_queue_Gubmo_Structs_Arrays_MyArrayInt32_1_Recv_init(
...
bool sb_queue_Gubmo_Structs_Arrays_MyArrayInt32_1_dequeue(
  ...
  sb_queue_Gubmo_Structs_Arrays_MyArrayInt32_1_t *queue = recvQueue->queue;
  ...
  // incremented. Thus there are really only (SB_QUEUE_GUBMO_STRUCTS_ARRAYS_MYARRAYINT32_1_SIZE - 1)
  // elements in the queue.
  *numDropped = (numNew <= SB_QUEUE_GUBMO_STRUCTS_ARRAYS_MYARRAYINT32_1_SIZE - 1) ? 0 : numNew - SB_QUEUE_GUBMO_STRUCTS_ARRAYS_MYARRAYINT32_1_SIZE + 1;
  ...
  *numRecv += *numDropped + 1;
  ...
  size_t index = (*numRecv - 1) % SB_QUEUE_GUBMO_STRUCTS_ARRAYS_MYARRAYINT32_1_SIZE;
  memcpy(data, &queue->elt[index], Gubmo_Structs_Arrays_MyArrayInt32_BYTE_SIZE); // Copy data

  // Acquire memory fence - ensure read of data BEFORE reading queue->numSent again
  __atomic_thread_fence(__ATOMIC_ACQUIRE);

  if (queue->numSent - *numRecv + 1 < SB_QUEUE_GUBMO_STRUCTS_ARRAYS_MYARRAYINT32_1_SIZE) {
...
bool sb_queue_Gubmo_Structs_Arrays_MyArrayInt32_1_is_empty(sb_queue_Gubmo_Structs_Arrays_MyArrayInt32_1_Recv_t *recvQueue) {

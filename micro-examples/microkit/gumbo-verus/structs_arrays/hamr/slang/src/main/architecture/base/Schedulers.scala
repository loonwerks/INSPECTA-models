...
object Schedulers {
  ...
  val threadNickNames: Map[String, Art.BridgeId] = Map(
    ISZ(
      Arch.Sys_i_Instance_producer_producer.name ~> Arch.Sys_i_Instance_producer_producer.id,
      Arch.Sys_i_Instance_consumer_consumer.name ~> Arch.Sys_i_Instance_consumer_consumer.id)
  )
  ...
  val Sys_i_Instance_proc_timingProperties: ProcessorTimingProperties = ProcessorTimingProperties(
    clockPeriod = Some(1),
    framePeriod = Some(1000),
    maxDomain = None(),
    slotTime = None())

  val Sys_i_Instance_producer_producer_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = None(),
    domain = Some(2))

  val Sys_i_Instance_consumer_consumer_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = None(),
    domain = Some(3))
  ...
  val defaultDomainToBridgeIdMap: ISZ[Art.BridgeId] = ISZ(
    /* domain 0 */ Arch.Sys_i_Instance_producer_producer.id,
    /* domain 1 */ Arch.Sys_i_Instance_consumer_consumer.id
  )

...
val componentsDir = microkitMcsDir / "components"
...
def rec(p: Os.Path, onlyDelAutoGen: B): Unit = {
  if (p.isSymLink) {
    // Never follow a symlink: p.list/p.remove would reach THROUGH it and delete
    // the real target. The aux_code/* links point into the r2u2 source tree, so
    // descending here is what was destroying r2u2 (KeepPath protects direct
    // traversal of r2u2, but not deletion coming in through these links).
    // Remove only the link itself (it is regenerated by codegen); its target is
    // left untouched.
    if (!keep(p) && !onlyDelAutoGen) {
      p.remove()
      println(s"Removed symlink: $p")
    }
  } else if (p.isFile) {

...
#
# Verus is tied to one exact Z3 version, and its x86 release ships that Z3
# alongside the binary -- so on x86 there is nothing to do here and this script
# is a no-op.  aarch64 has no Verus release at all, and Z3 publishes no aarch64
# Linux release either, so it is built here; verus.sh then links it in as
# ${VERUS_Z3_PATH} so both architectures end up with the same layout.
set -Eeuxo pipefail
...
if [ "${VERUS_FROM_SOURCE}" != "true" ]; then
  set +x
  echo "z3.sh: ${PROVERS_ARCH} uses the Z3 bundled with the Verus release; nothing to build"
  exit 0
fi

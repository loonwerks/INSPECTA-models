...
object Temperature_Sensor__InjectionProvider_Ext {
  ...
  def init(): Unit = {
    Temperature_Sensor_i_temperature_sensor_cpi_thermostat_Injection_Service.register(
      new Temperature_Sensor_i_temperature_sensor_cpi_thermostat_Injection_Provider {

        var lastTemperature = Isolette_Data_Model.PhysicalTemp_i(InitialValues.DEFAULT_CURRENT_TEMPERATURE)
        ...
        override def pre_receiveInput(): Unit = {
          ...
          art.Art.insertInInfrastructurePort(
            air_port_id,
            Isolette_Data_Model.PhysicalTemp_i_Payload(lastTemperature))
        }

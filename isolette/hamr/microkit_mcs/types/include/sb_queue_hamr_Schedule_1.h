...
// SB_QUEUE_HAMR_SCHEDULE_1_SIZE-1 elements.
#define SB_QUEUE_HAMR_SCHEDULE_1_SIZE 2
...
typedef struct sb_queue_hamr_Schedule_1 {
  ...
  // Queue of elements of type hamr_Schedule
  // (see sb_types.h) implemented as a ring buffer.
  // No initialization necessary.
  hamr_Schedule elt[SB_QUEUE_HAMR_SCHEDULE_1_SIZE];

} sb_queue_hamr_Schedule_1_t;
...
#define SB_QUEUE_HAMR_SCHEDULE_1_REGION_BYTES 4096
_Static_assert(sizeof(sb_queue_hamr_Schedule_1_t) <= SB_QUEUE_HAMR_SCHEDULE_1_REGION_BYTES,
  "sb_queue_hamr_Schedule_1_t outgrows its shared memory region");
...
void sb_queue_hamr_Schedule_1_init(sb_queue_hamr_Schedule_1_t *queue);

// Enqueue data. This always succeeds and never blocks. Data is copied.
void sb_queue_hamr_Schedule_1_enqueue(
  sb_queue_hamr_Schedule_1_t *queue,
  hamr_Schedule *data);
...
typedef struct sb_queue_hamr_Schedule_1_Recv {
...
} sb_queue_hamr_Schedule_1_Recv_t;

// Each receiver must call this exactly once before any calls to other queue
// API functions.
void sb_queue_hamr_Schedule_1_Recv_init(
  sb_queue_hamr_Schedule_1_Recv_t *recvQueue,
  sb_queue_hamr_Schedule_1_t *queue);
...
// dropped since the last call to sb_queue_hamr_Schedule_1_dequeue(). *data is left in
// unspecified state.
//
// If the sender ever gets ahead of a receiver by more than COUNTER_MAX,
// sb_queue_hamr_Schedule_1_dequeue will fail to count a multiple of COUNTER_MAX in
...
//
// An element holding an invalid bit pattern (see sb_queue_hamr_Schedule_1_numInvalid) is never
// copied to *data: the dequeue returns false and counts it.
bool sb_queue_hamr_Schedule_1_dequeue(
  sb_queue_hamr_Schedule_1_Recv_t *recvQueue,
  sb_event_counter_t *numDropped,
  hamr_Schedule *data);

// Number of elements this receiver has rejected as invalid.
static inline uintmax_t sb_queue_hamr_Schedule_1_numInvalid(sb_queue_hamr_Schedule_1_Recv_t *recvQueue) {
...
bool sb_queue_hamr_Schedule_1_is_empty(sb_queue_hamr_Schedule_1_Recv_t *recvQueue);

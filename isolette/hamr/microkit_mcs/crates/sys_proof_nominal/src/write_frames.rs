...
pub open spec fn mri_global_write_frame(pre: SystemState, post: SystemState) -> bool
{
  ...
  && pre.sys_assert_nominal_monitor_thread_sched_state == post.sys_assert_nominal_monitor_thread_sched_state
  && pre.sys_assert_nominal_monitor_thread_sched_schedule == post.sys_assert_nominal_monitor_thread_sched_schedule
}
...
/** SYS_ASSERT_NOMINAL_MONITOR_THREAD writes: nothing.
  * Everything else must be unchanged.
  */
pub open spec fn sys_assert_nominal_monitor_thread_global_write_frame(pre: SystemState, post: SystemState) -> bool
{
  pre.upper_desired_temp == post.upper_desired_temp
  && pre.lower_desired_temp == post.lower_desired_temp
  && pre.displayed_temp == post.displayed_temp
...
/** SYS_ASSERT_NOMINAL_MONITOR_THREAD local write frame: the component may modify anything in its
  * own scope (Isabelle `lFrame := (λ_ _. True)`).
  */
pub open spec fn sys_assert_nominal_monitor_thread_local_write_frame(pre: SystemState, post: SystemState) -> bool
{
  true
}

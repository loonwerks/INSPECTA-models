...
pub extern "C" fn thermostat_rt_mhs_mhs_timeTriggered() {
  ...
  let mut lower = isolette_Isolette_Data_Model_Temp_i { degrees: 0.0 };
  api::unsafe_get_lower_desired_temp(&mut lower);
  
  let mut upper = isolette_Isolette_Data_Model_Temp_i { degrees: 0.0 };
  api::unsafe_get_lower_desired_temp(&mut upper);

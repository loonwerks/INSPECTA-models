...
/** property error_condition_alarm_on, bound 'after mmm' (place after_mmm)
  *   any error condition (invalid alarm temp via REQ_MMI_4, invalid CT, or internal failure) -> mode out of Normal
  */
pub open spec fn sys_assert_error_condition_alarm_on_after_mmm(st: SystemState) -> bool
{
  ((((sysProp_REQ_MMI_4(st.lower_alarm_tempWstatus, st.upper_alarm_tempWstatus, st.mon_interface_failure) && sysProp_REQ_MMI_5(st.lower_alarm_tempWstatus, st.upper_alarm_tempWstatus, st.mon_interface_failure)) &&
    ...
    sysProp_ErrorConditionMonitorNotNormal(st.lower_alarm_tempWstatus, st.upper_alarm_tempWstatus, st.current_tempWstatus, st.mon_internal_failure, st.monitor_mode)
}

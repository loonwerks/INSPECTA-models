...
pub trait Manage_Regulator_Interface_Get_Api: Manage_Regulator_Interface_Api {
  ...
  #[verifier::external_body]
  fn get_regulator_mode_unverified(&self, data: &Ghost<isolette_Isolette_Data_Model_Regulator_Mode_Type>) -> 
  ...
  #[verifier::external_body]
  fn get_lower_desired_tempWstatus_unverified(&self, data: &Ghost<isolette_Isolette_Data_Model_TempWstatus_i>) -> 
    (res: isolette_Isolette_Data_Model_TempWstatus_i)
        ensures res.degrees == data@.degrees && res.status == data@.status {
    return super::extern_api::unsafe_get_lower_desired_tempWstatus();
  }

  #[verifier::external_body]
  fn get_upper_desired_tempWstatus_unverified(&self, data: &Ghost<isolette_Isolette_Data_Model_TempWstatus_i>) ->
     (res: isolette_Isolette_Data_Model_TempWstatus_i)
        ensures res.degrees == data@.degrees && res.status == data@.status {
  ...
  #[verifier::external_body]
  fn get_current_tempWstatus_unverified(&self, data: &Ghost<isolette_Isolette_Data_Model_TempWstatus_i>) -> 
    (res: isolette_Isolette_Data_Model_TempWstatus_i) 
        ensures res.degrees == data@.degrees && res.status == data@.status {

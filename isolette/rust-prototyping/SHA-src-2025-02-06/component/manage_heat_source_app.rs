...
impl Manage_Heat_Source_i_thermostat_rt {
    ...
    fn timeTriggered<API: Manage_Heat_Source_i_Full_Api>(&mut self, api: &mut Manage_Heat_Source_i_Application_Api<API>)
        // The requires clause is not needed because the condition
        //      api.lower_desired_temp.degrees <= api.upper_desired_temp.degrees
        // is assumes to be satisfied by the get-operations on the ports.
        // Because the values are read from the ports on each invocation, this is not a local invariant.
        // It can only be mainained on system level and must be verified outside this component.

...
@datatype class LowLevelEthernetDriver_Impl_Operational_Api (
  ...
  def get_EthernetFramesTx() : Option[SW.RawEthernetMessage] = {
    ...
    val value : Option[SW.RawEthernetMessage] = Art.getValue(EthernetFramesTx_Id) match {
      case Some(SW.RawEthernetMessage_Payload(v)) => Some(v)
      case Some(v) =>
        Art.logError(id, s"Unexpected payload on port EthernetFramesTx.  Expecting 'SW.RawEthernetMessage_Payload' but received ${v}")

...
verus! {
  ...
  impl Firewall_Firewall {
        ...
        // END MARKER TIME TRIGGERED ENSURES
    {
      ...
      match api.get_EthernetFramesRxIn() {
        Some(rxIn) => {
          if (compute_should_allow_inbound_frame_rx(rxIn)) {
      ...
      match api.get_EthernetFramesTxIn() {
        Some(txIn) => {
          if (compute_should_allow_outbound_frame_tx(txIn)) {
  ...
  exec fn log_warn_channel(channel: u32) {
    ...
    exec fn compute_should_allow_inbound_frame_rx(frame: SW::RawEthernetMessage) -> (result: bool)
        ensures
            should_allow_inbound_frame_rx(frame, result)
    {
    ...
    exec fn compute_should_allow_outbound_frame_tx(frame: SW::RawEthernetMessage) -> (result: bool)
        ensures
            should_allow_outbound_frame_tx(frame, result)
    {
    ...
    exec fn compute_frame_is_wellformed_eth2(frame: SW::RawEthernetMessage) -> (result: bool)
      ensures
        frame_is_wellformed_eth2(frame) <==> result,
    {
    ...
    exec fn compute_frame_has_ipv4_tcp(frame: SW::RawEthernetMessage) -> (result: bool)
      ...
      ensures
          frame_has_ipv4_tcp(frame) <==> result,
    {
    ...
    exec fn compute_frame_has_ipv4_tcp_on_allowed_port(frame: SW::RawEthernetMessage) -> (result: bool)
      ...
      ensures
          frame_has_ipv4_tcp_on_allowed_port(frame) <==> result,
    {
        // Deny by default
        ...
        */
        ...
        return compute_TCP_ALLOWED_PORTS()[0] == (256 * frame[36] as u16) + frame[37] as u16
    }
    ...
    exec fn compute_frame_has_ipv4_udp_on_allowed_port(frame: SW::RawEthernetMessage) -> (result: bool)
      ...
      ensures
        frame_has_ipv4_udp_on_allowed_port(frame) <==> result,

    {
        ...
        */
        ...
        return compute_UDP_ALLOWED_PORTS()[0] == (256 * frame[36] as u16) + frame[37] as u16
    }

    exec fn compute_two_bytes_to_u16(byte0: u8, byte1: u8) -> (res: u16)
      ensures res == two_bytes_to_u16(byte0, byte1)
    {
      return (((byte0) as u16) * 256u16 + ((byte1) as u16)) 
    }


    exec fn compute_TCP_ALLOWED_PORTS() -> (res: SW::u16Array)
      ensures res == TCP_ALLOWED_PORTS()
    {
       [5760u16, 0u16, 0u16, 0u16]
    }

    exec fn compute_UDP_ALLOWED_PORTS() -> (res: SW::u16Array)
      ensures res == UDP_ALLOWED_PORTS()
    {
  ...
  pub open spec fn frame_is_wellformed_eth2(eth_frame: SW::RawEthernetMessage) -> bool
  {
    if (!((eth_frame[12] >= 6u8) &&
      (eth_frame[13] >= 0u8))) {
      false
  ...
  pub open spec fn frame_has_ipv4(eth_frame: SW::RawEthernetMessage) -> bool
  {
  ...
  pub open spec fn frame_has_ipv4_tcp(eth_frame: SW::RawEthernetMessage) -> bool
  {
  ...
  pub open spec fn frame_has_ipv4_udp(eth_frame: SW::RawEthernetMessage) -> bool
  {
    frame_is_wellformed_eth2(eth_frame) && frame_has_ipv4(eth_frame) ==>
  ...
  pub open spec fn frame_has_ipv4_tcp_on_allowed_port(eth_frame: SW::RawEthernetMessage) -> bool
  {
    frame_is_wellformed_eth2(eth_frame) && frame_has_ipv4(eth_frame) &&
  ...
  pub open spec fn frame_has_ipv4_tcp_on_allowed_port_quant(eth_frame: SW::RawEthernetMessage) -> bool
  {
    exists|i:int| 0 <= i < TCP_ALLOWED_PORTS().len() && #[trigger] TCP_ALLOWED_PORTS()[i] == two_bytes_to_u16(eth_frame[36], eth_frame[37])
  }

  pub open spec fn frame_has_ipv4_udp_on_allowed_port(eth_frame: SW::RawEthernetMessage) -> bool
  {
    frame_is_wellformed_eth2(eth_frame) && frame_has_ipv4(eth_frame) &&
  ...
  pub open spec fn frame_has_ipv4_udp_on_allowed_port_quant(eth_frame: SW::RawEthernetMessage) -> bool
  {
    exists|i:int| 0 <= i < UDP_ALLOWED_PORTS().len() && #[trigger] UDP_ALLOWED_PORTS()[i] == two_bytes_to_u16(eth_frame[36], eth_frame[37])
  }

  pub open spec fn frame_has_ipv6(eth_frame: SW::RawEthernetMessage) -> bool
  {
    frame_is_wellformed_eth2(eth_frame) ==>
  ...
  pub open spec fn frame_has_arp(eth_frame: SW::RawEthernetMessage) -> bool
  {
    frame_is_wellformed_eth2(eth_frame) ==>
      (if (!((eth_frame[12] == 8u8) &&
        (eth_frame[13] == 6u8))) {

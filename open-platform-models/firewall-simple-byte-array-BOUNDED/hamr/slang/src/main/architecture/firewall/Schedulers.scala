...
object Schedulers {
  ...
  val threadNickNames: Map[String, Art.BridgeId] = Map(
    ISZ(
      Arch.seL4_Impl_Instance_ArduPilot_ArduPilot.name ~> Arch.seL4_Impl_Instance_ArduPilot_ArduPilot.id,
      Arch.seL4_Impl_Instance_Firewall_Firewall.name ~> Arch.seL4_Impl_Instance_Firewall_Firewall.id,
      Arch.seL4_Impl_Instance_LowLevelEthernetDriver_LowLevelEthernetDriver.name ~> Arch.seL4_Impl_Instance_LowLevelEthernetDriver_LowLevelEthernetDriver.id)
  )
  ...
  val seL4_Impl_Instance_proc_timingProperties: ProcessorTimingProperties = ProcessorTimingProperties(
    clockPeriod = Some(1),
    framePeriod = Some(1000),
    maxDomain = None(),
    slotTime = None())

  val seL4_Impl_Instance_ArduPilot_ArduPilot_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = Some((100, 100)),
    domain = Some(4))

  val seL4_Impl_Instance_Firewall_Firewall_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = Some((100, 100)),
    domain = Some(3))

  val seL4_Impl_Instance_LowLevelEthernetDriver_LowLevelEthernetDriver_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = Some((100, 100)),
    domain = Some(2))
  ...
  val defaultDomainToBridgeIdMap: ISZ[Art.BridgeId] = ISZ(
    /* domain 0 */ Arch.seL4_Impl_Instance_ArduPilot_ArduPilot.id,
    /* domain 1 */ Arch.seL4_Impl_Instance_Firewall_Firewall.id,
    /* domain 2 */ Arch.seL4_Impl_Instance_LowLevelEthernetDriver_LowLevelEthernetDriver.id
  )

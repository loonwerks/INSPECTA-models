...
sb_queue_SW_StructuredEthernetMessage_i_1_Recv_t EthernetFramesRx_recv_queue;

#define PORT_FROM_MON 60

bool put_EthernetFramesTx(const SW_StructuredEthernetMessage_i *data) {
  sb_queue_SW_StructuredEthernetMessage_i_1_enqueue((sb_queue_SW_StructuredEthernetMessage_i_1_t *) EthernetFramesTx_queue_1, (SW_StructuredEthernetMessage_i *) data);

  return true;
}

bool EthernetFramesRx_is_empty(void) {
  return sb_queue_SW_StructuredEthernetMessage_i_1_is_empty(&EthernetFramesRx_recv_queue);
}

bool get_EthernetFramesRx_poll(sb_event_counter_t *numDropped, SW_StructuredEthernetMessage_i *data) {
  return sb_queue_SW_StructuredEthernetMessage_i_1_dequeue((sb_queue_SW_StructuredEthernetMessage_i_1_Recv_t *) &EthernetFramesRx_recv_queue, numDropped, data);
}
...
void init(void) {
  ...
  sb_queue_SW_StructuredEthernetMessage_i_1_Recv_init(&EthernetFramesRx_recv_queue, (sb_queue_SW_StructuredEthernetMessage_i_1_t *) EthernetFramesRx_queue_1);

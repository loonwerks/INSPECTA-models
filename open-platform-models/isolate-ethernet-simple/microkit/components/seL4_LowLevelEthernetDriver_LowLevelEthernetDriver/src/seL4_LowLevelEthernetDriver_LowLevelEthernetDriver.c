...
sb_queue_base_SW_RawEthernetMessage_Impl_1_Recv_t EthernetFramesTx_recv_queue;
volatile sb_queue_base_SW_RawEthernetMessage_Impl_1_t *EthernetFramesRx_queue_1;

#define PORT_FROM_MON 56

base_SW_RawEthernetMessage_Impl last_EthernetFramesTx_payload;

bool get_EthernetFramesTx(base_SW_RawEthernetMessage_Impl *data) {
  sb_event_counter_t numDropped;
  base_SW_RawEthernetMessage_Impl fresh_data;
  bool isFresh = sb_queue_base_SW_RawEthernetMessage_Impl_1_dequeue((sb_queue_base_SW_RawEthernetMessage_Impl_1_Recv_t *) &EthernetFramesTx_recv_queue, &numDropped, &fresh_data);
  if (isFresh) {
    memcpy(&last_EthernetFramesTx_payload, &fresh_data, base_SW_RawEthernetMessage_Impl_SIZE);
  }
  memcpy(data, &last_EthernetFramesTx_payload, base_SW_RawEthernetMessage_Impl_SIZE);
  return isFresh;
}
...
void init(void) {
  sb_queue_base_SW_RawEthernetMessage_Impl_1_Recv_init(&EthernetFramesTx_recv_queue, (sb_queue_base_SW_RawEthernetMessage_Impl_1_t *) EthernetFramesTx_queue_1);
